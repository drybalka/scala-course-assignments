...
trait Huffman extends HuffmanInterface:
  ...
  def times(chars: List[Char]): List[(Char, Int)] =
    def increment(char: Char, list: List[(Char, Int)]): List[(Char, Int)] =
      list match
        case (c, count) :: tail if c == char => (char, count + 1) :: tail
        case head :: tail                    => head :: increment(char, tail)
        case Nil                             => (char, 1) :: Nil

    chars match
      case char :: tail => increment(char, times(tail))
      case Nil          => Nil
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] =
    // def insert(char: Char, weight: Int, list: List[Leaf]): List[Leaf] =
    //   list match
    //     case Leaf(otherChar, otherWeight) :: tail =>
    //       if otherWeight < weight then
    //         Leaf(otherChar, otherWeight) :: insert(char, weight, tail)
    //       else Leaf(char, weight) :: Leaf(otherChar, otherWeight) :: tail
    //     case Nil => Leaf(char, weight) :: Nil

    freqs match
      case (char, weight) :: tail =>
        insertInOrder(Leaf(char, weight), makeOrderedLeafList(tail))
      case Nil => Nil

  def insertInOrder[T <: CodeTree](element: T, list: List[T]): List[T] =
    list match
      case head :: tail =>
        if weight(head) < weight(element) then
          head :: insertInOrder(element, tail)
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] =
    ...
    def decodeProcess(tree: CodeTree, bits: List[Bit]): List[Char] =
      tree match
        case Leaf(char, _) => char :: decode(originalTree, bits)
        case Fork(left, right, _, _) =>
          bits match
            case 0 :: tail => decodeProcess(left, tail)
            case 1 :: tail => decodeProcess(right, tail)
            case Nil       => Nil
            case _         => ???

    decodeProcess(tree, bits)
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] =
    val originalTree = tree
    def encodeProcess(tree: CodeTree, text: List[Char]): List[Bit] =
      text match
        case Nil => Nil
        case char :: tail =>
          tree match
            case Leaf(char, _) => encode(originalTree)(tail)
            case Fork(left, right, _, _) =>
              if chars(left) contains char then 0 :: encodeProcess(left, text)
              else 1 :: encodeProcess(right, text)

    encodeProcess(tree, text)
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] =
    table match
      case (entry, representation) :: tail =>
        if entry == char then representation else codeBits(tail)(char)
      case _ => ???
  ...
  def convert(tree: CodeTree): CodeTable =
    tree match
      ...
      case Fork(left, right, _, _) =>
        convert(left).map((char, bits) => (char, 0 :: bits)) ::: convert(right)
          .map((char, bits) => (char, 1 :: bits))
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = 
    def coder = codeBits(convert(tree))
    text.flatMap(coder)
