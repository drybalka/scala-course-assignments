...
class Empty extends TweetSet:
  ...
  def mostRetweeted: Tweet = throw ju.NoSuchElementException()
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet =
    val accUpdated = if p(elem) then acc.incl(elem) else acc
    right.filterAcc(p, left.filterAcc(p, accUpdated))

  def union(that: TweetSet): TweetSet =
    right.union(left.union(that.incl(elem)))

  def mostRetweeted: Tweet =
    def moreRetweeted(tweet: Tweet, set: TweetSet): Tweet =
      set match
        case _: Empty => tweet
        case _: NonEmpty =>
          val other = set.mostRetweeted
          if other.retweets > tweet.retweets then other else tweet

    moreRetweeted(moreRetweeted(elem, left), right)

  def descendingByRetweet: TweetList =
    val tweet = this.mostRetweeted
    val reducedSet = this.remove(tweet)
    Cons(tweet, reducedSet.descendingByRetweet)
...
object GoogleVsApple:
  ...
  def tweetPicker(tweets: TweetSet, dict: List[String]): TweetSet =
    tweets.filter(tweet => dict.exists(word => tweet.text.contains(word)))

  lazy val googleTweets: TweetSet = tweetPicker(TweetReader.allTweets, google)
  lazy val appleTweets: TweetSet = tweetPicker(TweetReader.allTweets, apple)
