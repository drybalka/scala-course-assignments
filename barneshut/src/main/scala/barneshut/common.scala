  ...
  def updated(quad: Quad): Body =
    ...
    def traverse(quad: Quad): Unit = (quad: Quad) match
      ...
      case Leaf(_, _, _, bodies) =>
        ...
        for b <- bodies do addForce(b.mass, b.x, b.y)
...
class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int)
  ...
  def +=(b: Body): SectorMatrix =
    val x =
      ...
      else ((b.x - boundaries.minX) / sectorSize).floor.toInt
    val y =
      if b.y < boundaries.minY then 0
      else if b.y > boundaries.maxY then sectorPrecision - 1
      else ((b.y - boundaries.minY) / sectorSize).floor.toInt
    this(x, y) += b
    this
