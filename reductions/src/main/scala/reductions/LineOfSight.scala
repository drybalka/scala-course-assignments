...
object LineOfSight extends LineOfSightInterface:
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit =
    output(0) = 0
    def step(i: Int, maxAngle: Float): Unit =
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float =
    @tailrec
    def step(i: Int, res: Float): Float =
      if (i == until) then res else step(i + 1, res max input(i) / i)
    step(from, 0.0)
  ...
  def parLineOfSight(
    ...
    downsweep(input, output, 0.0, tree)
