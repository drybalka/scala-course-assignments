...
object Anagrams extends AnagramsInterface:
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =
    occurrences match
      ...
      case (char, occurrence) :: tail =>
        ...
        tailCombinations ::: headCombinations
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences =
    val subMap = y.toMap
    x.map((char, occurrence) => (char, occurrence - subMap.getOrElse(char, 0)))
      .filter((_, occurrence) => occurrence > 0)
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] =
    def occurrenceAnagrams(occurrence: Occurrences): List[Sentence] =
      if occurrence == Nil then List(Nil)
      else
        combinations(occurrence).flatMap(occ =>
          for
            word <- dictionaryByOccurrences.getOrElse(occ, Nil)
            sentence <- occurrenceAnagrams(subtract(occurrence, occ))
          yield word :: sentence
        )
