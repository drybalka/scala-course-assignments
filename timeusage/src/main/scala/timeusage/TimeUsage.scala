...
object TimeUsage extends TimeUsageInterface:
  ...
  def classifiedColumns(
    ...
    val grouped = columnNames.groupMap(name =>
      name.take(3) match
        case "t01" | "t03" | "t11" => "primary"
        case "t05"                 => "work"
        case "t02" | "t04" | "t06" | "t07" | "t08" | "t09" | "t10" | "t12" |
            "t13" | "t14" | "t15" | "t16" =>
          "leisure"
        case "t18" =>
          name.take(5) match
            case "t1801" | "t1803" => "primary"
            case "t1805"           => "work"
            case _                 => "leisure"
        case _ => "rest"
    )(col)
    (grouped("primary"), grouped("work"), grouped("leisure"))
  ...
  def timeUsageGroupedSqlQuery(viewName: String): String =
    f"""SELECT working, sex, age, ROUND(AVG(primaryNeeds), 1), ROUND(AVG(work), 1), ROUND(AVG(other), 1) 
    FROM $viewName 
