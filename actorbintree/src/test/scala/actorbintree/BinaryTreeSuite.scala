  ...
  class BinaryTreeSuite extends munit.FunSuite:
    ...
    class Impl
      ...
      test("myTest") {
        ...
        topNode ! Insert(requester.ref, id = 0, 0)
        // topNode ! Insert(requester.ref, id = -1, -1)
        topNode ! Insert(requester.ref, id = 1, 1)

        // topNode ! Remove(requester.ref, id = 10, 0)
        // topNode ! Remove(requester.ref, id = -11, -1)
        topNode ! Remove(requester.ref, id = 11, 1)

        topNode ! GC

        topNode ! Contains(requester.ref, id = 20, 1)

        requester.receiveN(4).foreach(println)
