...
object Server extends ServerModuleInterface:
  ...
  val reframedFlow: Flow[ByteString, String, NotUsed] =
    Framing.delimiter(delimiter = ByteString("\n"), maximumFrameLength = 100).map(_.decodeString(Charset.defaultCharset()))
  ...
  val reintroduceOrdering: Flow[Event, Event, NotUsed] =
    Flow[Event].statefulMapConcat( () =>
      var next = 1
      var buffer = Map.empty[Int, Event]

      def nextElementsFromBuffer(): List[Event] =
        if buffer.contains(next) then
          val nextElement = buffer(next)
          buffer -= next
          next += 1
          nextElement :: nextElementsFromBuffer()
        else
          Nil

      event =>
        if event.sequenceNr == next then
          next += 1
          event :: nextElementsFromBuffer()
        else
  ...
  val followersFlow: Flow[Event, (Event, Followers), NotUsed] =
    Flow[Event].statefulMapConcat( () =>
      ...
      event =>
        ...
        Iterable.single((event, followers))
    )
  ...
  def isNotified(userId: Int)(eventAndFollowers: (Event, Followers)): Boolean =
    ...
    event match
      ...
      case StatusUpdate(_, fromUserId) => followers.get(userId).map(_.contains(fromUserId)).getOrElse(false)
...
class Server(using ExecutionContext, Materializer)
  ...
  def clientFlow(): Flow[ByteString, ByteString, NotUsed] =
    ...
    val incoming: Sink[ByteString, NotUsed] =
      identityParserSink.mapMaterializedValue(futureId =>
        clientIdPromise.completeWith(futureId)
        NotUsed
      )
