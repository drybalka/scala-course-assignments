...
object SelectiveReceive:
  ...
  private def intercept[T: ClassTag](bufferSize: Int, buffer: StashBuffer[T], started: Behavior[T]): Behavior[T] =
    Behaviors.receive { case (ctx, message) =>
      ...
      if Behavior.isUnhandled(next) then
        // println(s"Cannot handle $message")
        buffer.stash(message)
        // println(s"Buffer is now $buffer")
        Behaviors.same
      else
        // println(s"Handled $message")
        val nextCanonicalized = Behavior.canonicalize(next, started, ctx)
