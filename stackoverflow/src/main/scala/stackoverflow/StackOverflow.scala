...
class StackOverflow extends StackOverflowInterface with Serializable:
  ...
  def groupedPostings(
    ...
    val questions =
      postings.filter(_.postingType == 1).map(posting => (posting.id, posting))
  ...
  def scoredPostings(
    ...
    grouped.values
      .map(_.unzip)
      .map((qs, as) => (qs.head, answerHighScore(as.toArray)))
  ...
  def vectorPostings(
    ...
    scored
      .map((q, hs) => (firstLangInTag(q.tags, langs).map(_ * langSpread), hs))
  ...
  def clusterResults(
    ...
    val median = closestGrouped.mapValues { vs =>
      val langIndex =
        vs.groupMapReduce(_._1)(_ => 1)(_ + _).maxBy(_._2)._1
      val langLabel: String =
        langs(langIndex / langSpread) // most common language in the cluster
      // percent of the questions in the most common language
      val langPercent: Double =
        vs.count(_._1 == langIndex) * 100.0 / vs.size
      val clusterSize: Int = vs.size
      val sortedScores = vs.map(_._2).toArray.sortInPlace()
      val medianScore: Int =
        if clusterSize % 2 == 1 then sortedScores(clusterSize / 2)
        else
          (sortedScores(clusterSize / 2) +
            sortedScores(clusterSize / 2 - 1)) / 2
